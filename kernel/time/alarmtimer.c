...
void alarm_get_power_on(struct timespec *new_pwron_time)
{
	struct rtc_wkalrm pwron_alm;

	memset(&pwron_alm, 0, sizeof(struct rtc_wkalrm));
	rtc_read_pwron_alarm(&pwron_alm);
	rtc_tm_to_time(&pwron_alm.time, &new_pwron_time->tv_sec);
}
