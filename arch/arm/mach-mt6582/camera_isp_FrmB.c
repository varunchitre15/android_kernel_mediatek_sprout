...
static long ISP_Buf_CTRL_FUNC_FRMB(MUINT32 Param)
{
    ...
    //
    if(copy_from_user(&rt_buf_ctrl, (void*)Param, sizeof(ISP_BUFFER_CTRL_STRUCT_FRMB)) == 0)
    {
...
//            case ISP_RT_BUF_CTRL_EXCHANGE_ENQUE:
                //
                if(copy_from_user(&rt_buf_info, (void*)rt_buf_ctrl.data_ptr, sizeof(ISP_RT_BUF_INFO_STRUCT_FRMB)) == 0) {
...
static MINT32 ISP_WaitIrq_FrmB(ISP_WAIT_IRQ_STRUCT_FRMB* WaitIrq)
{
    ...
    LOG_DBG(" [WAITIRQ] mark sec/usec (%d/%d), irq sec/usec (%d/%d),user(0x%x)",WaitIrq->TimeInfo.tMark2WaitSig_sec,WaitIrq->TimeInfo.tMark2WaitSig_usec,\
