...
/* defaults */
#define Dl1Bt_MAX_BUFFER_SIZE     (24*1024)
#define MIN_PERIOD_SIZE       64
#define MAX_PERIOD_SIZE     Dl1Bt_MAX_BUFFER_SIZE
...
static struct snd_pcm_hardware mtk_dl1bt_pcm_hardware = {
	...
	.buffer_bytes_max = Dl1Bt_MAX_BUFFER_SIZE,
...
static snd_pcm_uframes_t mtk_dl1bt_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	pr_debug("mtk_dl1bt_pcm_pointer\n");

	if (pdl1btMemControl->interruptTrigger == 1) {
		...
		pdl1btMemControl->interruptTrigger = 0;
		return (HW_memory_index >> 2);
	}
	PRINTK_AUD_DL1("mtk_dl1bt_pcm_pointer Previous_Hw_cur = 0x%x\n", Previous_Hw_cur);
...
static unsigned int dl1bt_supported_sample_rates[] = {
	8000, 16000
};

static struct snd_pcm_hw_constraint_list constraints_dl1_sample_rates = {
	.count = ARRAY_SIZE(dl1bt_supported_sample_rates),
	.list = dl1bt_supported_sample_rates,
	.mask = 0,
};
			      ...
			      void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (copy_size != 0) {
		...
		if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize)	/* copy once */
		{
			...
			PRINTK_AUD_DL1
			    ("AudDrv_write finish1, copy_size:%x, WriteIdx:%x, ReadIdx=%x, DataRemained:%x, count=%lu \r\n",
