			     ...
			     void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (copy_size != 0) {
		...
		if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize)	/* copy once */
		{
			...
			PRINTK_AUD_FMTX
			    ("[mtk_pcm_fmtx_copy] finish1, copy_size:%x, WriteIdx:%x, ReadIdx=%x, DataRemained:%x, count=%x \r\n",
		...
		} else		/* copy twice */
		{
			...
			if (!access_ok(VERIFY_READ, data_w_ptr, size_1)) {
				pr_debug
				    ("[mtk_pcm_fmtx_copy] 1ptr invalid data_w_ptr=%x, size_1=%d",
				...
				PRINTK_AUD_FMTX
				    ("[mtk_pcm_fmtx_copy]mcmcpy Afe_Block->pucVirtBufAddr+Afe_WriteIdx= %x data_w_ptr = %p size_1 = %x\n",
				     Afe_Block->pucVirtBufAddr + Afe_WriteIdx_tmp, data_w_ptr,
				     size_1);
				if ((copy_from_user
				     ((Afe_Block->pucVirtBufAddr + Afe_WriteIdx_tmp), data_w_ptr,
			...
			if (!access_ok(VERIFY_READ, data_w_ptr + size_1, size_2)) {
				...
				PRINTK_AUD_FMTX
				    ("[mtk_pcm_fmtx_copy]mcmcpy Afe_Block->pucVirtBufAddr+Afe_WriteIdx= %x data_w_ptr+size_1 = %p size_2 = %x\n",
