				     ...
				     struct snd_pcm_hw_params *hw_params)
{
	...
	if (Capture_dma_buf->area) {
		pr_debug("mtk_capture_pcm_hw_params Capture_dma_buf->area\n");
	...
	pr_debug("mtk_capture_pcm_hw_params dma_bytes = %d dma_area = %p dma_addr = 0x%x\n",
				...
				void __user *dst, snd_pcm_uframes_t count)
{
	...
	PRINTK_AUD_UL1
	    ("AudDrv_MEMIF_Read finish0, read_count:%x, read_size:%x, u4DataRemained:%x, u4DMAReadIdx:0x%x, u4WriteIdx:%x \r\n",
	...
	if (DMA_Read_Ptr + read_size < Vul_Block->u4BufferSize) {
		...
		PRINTK_AUD_UL1
		    ("AudDrv_MEMIF_Read finish1, copy size:%x, u4DMAReadIdx:0x%x, u4WriteIdx:%x, u4DataRemained:%x \r\n",
		...
		PRINTK_AUD_UL1
		    ("AudDrv_MEMIF_Read finish2, copy size_1:%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x, u4DataRemained:%x \r\n",
		...
		PRINTK_AUD_UL1
		    ("AudDrv_MEMIF_Read finish3, copy size_2:%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x u4DataRemained:%x \r\n",
