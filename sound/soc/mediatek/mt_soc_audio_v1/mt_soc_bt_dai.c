...
#define MAX_PERIOD_SIZE     BT_DAI_MAX_BUFFER_SIZE
				    ...
				    struct snd_pcm_hw_params *hw_params)
{
	...
	if (Bt_Dai_Capture_dma_buf->area) {
		...
		runtime->dma_bytes = Bt_Dai_Capture_dma_buf->bytes;
		runtime->dma_area = Bt_Dai_Capture_dma_buf->area;
		runtime->dma_addr = Bt_Dai_Capture_dma_buf->addr;
		runtime->buffer_size = Bt_Dai_Capture_dma_buf->bytes;
	} else {
...
static unsigned int bt_dai_supported_sample_rates[] = {
	8000, 11025, 12000, 16000, 22050, 24000, 32000
};

static struct snd_pcm_hw_constraint_list bt_dai_constraints_sample_rates = {
	.count = ARRAY_SIZE(bt_dai_supported_sample_rates),
	.list = bt_dai_supported_sample_rates,
};
			       ...
			       void __user *dst, snd_pcm_uframes_t count)
{
	...
	unsigned long flags;
	count = count * 2 * substream->runtime->channels;
	...
	pr_debug
	    ("AudDrv_MEMIF_Read finish0, read_count:0x%x, read_size:0x%x, u4DataRemained:0x%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x \r\n",
	     read_count, read_size, Dai_Block->u4DataRemained, Dai_Block->u4DMAReadIdx,
	     Dai_Block->u4WriteIdx);
	...
	if (DMA_Read_Ptr + read_size < Dai_Block->u4BufferSize) {
		if (DMA_Read_Ptr != Dai_Block->u4DMAReadIdx) {
			pr_debug
			    ("AudDrv_MEMIF_Read 1, read_size:0x%x, DataRemained:0x%x, DMA_Read_Ptr:0x%x, DMAReadIdx:0x%x \r\n",
			     read_size, Dai_Block->u4DataRemained, DMA_Read_Ptr,
		...
		if (copy_to_user
			...
			pr_debug
			    ("AudDrv_MEMIF_Read Fail 1 copy to user Read_Data_Ptr:%p, pucVirtBufAddr:%p, u4DMAReadIdx:0x%x, DMA_Read_Ptr:0x%x,read_size:%x",
			     Read_Data_Ptr, Dai_Block->pucVirtBufAddr, Dai_Block->u4DMAReadIdx,
		...
		pr_debug
		    ("AudDrv_MEMIF_Read finish1, copy size:0x%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x, u4DataRemained:0x%x \r\n",
		     read_size, Dai_Block->u4DMAReadIdx, Dai_Block->u4WriteIdx,
		...
		if (DMA_Read_Ptr != Dai_Block->u4DMAReadIdx) {
			...
			pr_debug
			    ("AudDrv_MEMIF_Read 2, read_size1:0x%x, DataRemained:0x%x, DMA_Read_Ptr:0x%x, DMAReadIdx:0x%x \r\n",
			     size_1, Dai_Block->u4DataRemained, DMA_Read_Ptr,
			     Dai_Block->u4DMAReadIdx);
		}
		if (copy_to_user
		    ((void __user *)Read_Data_Ptr, (Dai_Block->pucVirtBufAddr + DMA_Read_Ptr),
		...
		pr_debug
		    ("AudDrv_MEMIF_Read finish2, copy size_1:0x%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x, u4DataRemained:0x%x \r\n",
		     size_1, Dai_Block->u4DMAReadIdx, Dai_Block->u4WriteIdx,
		     Dai_Block->u4DataRemained);

		if (DMA_Read_Ptr != Dai_Block->u4DMAReadIdx) {

			pr_debug
			    ("AudDrv_DAIBT_Read 3, read_size2:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
			     size_2, Dai_Block->u4DataRemained, DMA_Read_Ptr,
			     Dai_Block->u4DMAReadIdx);
		}
		if (copy_to_user
		...
		pr_debug
		    ("AudDrv_MEMIF_Read finish3, copy size_2:0x%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x u4DataRemained:0x%x \r\n",
		     size_2, Dai_Block->u4DMAReadIdx, Dai_Block->u4WriteIdx,
		     Dai_Block->u4DataRemained);
	}
