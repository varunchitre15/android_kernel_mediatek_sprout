...
static struct pcm_afe_info *prtdtemp;
...
static int mtkalsa_awb_constraints(struct snd_pcm_runtime *runtime)
{
	...
	if (err < 0) {
		pr_debug("mtkalsa_awb_constraints SNDRV_PCM_HW_PARAM_PERIODS err = %d", err);
		return err;
	}
	err = snd_pcm_hw_constraint_minmax(runtime, SNDRV_PCM_HW_PARAM_BUFFER_BYTES, 1, UINT_MAX);
	if (err < 0) {
		pr_debug("snd_pcm_hw_constraint_minmax SNDRV_PCM_HW_PARAM_BUFFER_BYTES err = %d",
			 err);
		return err;
	}
	return 0;
...
static void StartAudioAWBHardware(struct snd_pcm_substream *substream)
{
	pr_debug("StartAudioAWBHardware\n");
...
void Auddrv_AWB_Interrupt_Handler(void)	/* irq1 ISR handler */
{
	...
	pr_debug("Auddrv_AWB_Interrupt_Handler\n ");
...
{
	pr_debug("mtk_awb_pcm_prepare substream->rate = %d  substream->channels = %d\n",
...
{
	pr_debug("mtk_awb_pcm_hw_params\n");
	struct snd_pcm_runtime *runtime = substream->runtime;
	...
	/*
	   ...
	   if (!dma_buf->area)
	   {
	   pr_debug("%s:mtk_awb_pcm_hw_params failed\n", __func__);
	   return -ENOMEM;
	   } */

	if (Awb_Capture_dma_buf->area) {
		pr_debug("mtk_awb_pcm_hw_params Awb_Capture_dma_buf->area\n");
		...
		pr_debug("mtk_awb_pcm_hw_params snd_pcm_lib_malloc_pages\n");
		ret = snd_pcm_lib_malloc_pages(substream, params_buffer_bytes(hw_params));
	}
	pr_debug("mtk_awb_pcm_hw_params dma_bytes = %d dma_area = %p dma_addr = 0x%x\n",
...
static int mtk_awb_pcm_open(struct snd_pcm_substream *substream)
{
	...
	memcpy((void *)(&(runtime->hw)), (void *)&mtk_awb_hardware,
	       sizeof(struct snd_pcm_hardware));
	runtime->private_data = prtd;

	pr_debug("runtime->hw->rates = 0x%x mtk_awb_hardware = = 0x%x\n ", runtime->hw.rates,
		 &mtk_awb_hardware);
	...
	pr_debug("mtk_awb_pcm_open runtime rate = %d channels = %d\n", runtime->rate,
		 runtime->channels);
	pr_debug("mtk_awb_pcm_open substream->pcm->device  %d\n", substream->pcm->device);
	...
	if (err < 0) {
		pr_debug("mtk_awb_pcm_close\n");
		mtk_awb_pcm_close(substream);
		return err;
	}
	pr_debug("mtk_awb_pcm_open return\n");
	return 0;
}
...
{
	pr_debug("mtk_awb_pcm_copy pos = %d count = %d\n ", pos, count);
	count = count << 2;
	...
	if (DMA_Read_Ptr + read_size < Awb_Block->u4BufferSize) {
		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			pr_debug
			    ("AudDrv_MEMIF_Read 1, read_size:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
		...
		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			...
			pr_debug
			    ("AudDrv_MEMIF_Read 2, read_size1:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
...
static int mtk_afe_awb_probe(struct snd_soc_platform *platform)
{
	...
	pr_debug("mtk_afe_awb_probe dma_alloc_coherent\n");
