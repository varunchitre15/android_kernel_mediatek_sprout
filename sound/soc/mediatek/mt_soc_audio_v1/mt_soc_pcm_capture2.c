				 ...
				 void __user *dst, snd_pcm_uframes_t count)
{
	...
	PRINTK_AUD_UL2
	    ("mtk_capture2_pcm_copy finish0, read_count:%x, read_size:%x, u4DataRemained:%x, u4DMAReadIdx:0x%x, u4WriteIdx:%x \r\n",
	     read_count, read_size, Vul_Block->u4DataRemained, Vul_Block->u4DMAReadIdx,
	     Vul_Block->u4WriteIdx);

	if (DMA_Read_Ptr + read_size < Vul_Block->u4BufferSize) {
		if (DMA_Read_Ptr != Vul_Block->u4DMAReadIdx) {
			pr_debug
			    ("mtk_capture2_pcm_copy 1, read_size:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
			     read_size, Vul_Block->u4DataRemained, DMA_Read_Ptr,
		...
		if (copy_to_user
			...
			pr_debug
			    ("mtk_capture2_pcm_copy Fail 1 copy to user Read_Data_Ptr:%p, pucVirtBufAddr:%p, u4DMAReadIdx:0x%x, DMA_Read_Ptr:0x%x,read_size:%x",
			     Read_Data_Ptr, Vul_Block->pucVirtBufAddr, Vul_Block->u4DMAReadIdx,
		...
		PRINTK_AUD_UL2
		    ("mtk_capture2_pcm_copy finish1, copy size:%x, u4DMAReadIdx:0x%x, u4WriteIdx:%x, u4DataRemained:%x \r\n",
		     read_size, Vul_Block->u4DMAReadIdx, Vul_Block->u4WriteIdx,
		...
		if (DMA_Read_Ptr != Vul_Block->u4DMAReadIdx) {
			...
			pr_debug
			    ("mtk_capture2_pcm_copy 2, read_size1:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
			     size_1, Vul_Block->u4DataRemained, DMA_Read_Ptr,
			     Vul_Block->u4DMAReadIdx);
		}
		if (copy_to_user
		    ((void __user *)Read_Data_Ptr, (Vul_Block->pucVirtBufAddr + DMA_Read_Ptr),
			...
			pr_debug
			    ("mtk_capture2_pcm_copy Fail 2 copy to user Read_Data_Ptr:%p, pucVirtBufAddr:%p, u4DMAReadIdx:0x%x, DMA_Read_Ptr:0x%x,read_size:%x",
			     Read_Data_Ptr, Vul_Block->pucVirtBufAddr, Vul_Block->u4DMAReadIdx,
			     DMA_Read_Ptr, read_size);
			return 0;
		}
		...
		PRINTK_AUD_UL2
		    ("mtk_capture2_pcm_copy finish2, copy size_1:%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x, u4DataRemained:%x \r\n",
		     size_1, Vul_Block->u4DMAReadIdx, Vul_Block->u4WriteIdx,
		     Vul_Block->u4DataRemained);

		if (DMA_Read_Ptr != Vul_Block->u4DMAReadIdx) {

			pr_debug
			    ("mtk_capture2_pcm_copy 3, read_size2:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
			     size_2, Vul_Block->u4DataRemained, DMA_Read_Ptr,
			     Vul_Block->u4DMAReadIdx);
		}
		if (copy_to_user
			...
			pr_debug
			    ("mtk_capture2_pcm_copy Fail 3 copy to user Read_Data_Ptr:%p, pucVirtBufAddr:%p, u4DMAReadIdx:0x%x , DMA_Read_Ptr:0x%x, read_size:%x",
			     Read_Data_Ptr, Vul_Block->pucVirtBufAddr, Vul_Block->u4DMAReadIdx,
			     DMA_Read_Ptr, read_size);
			return read_count << 2;
		}
		...
		PRINTK_AUD_UL2
		    ("mtk_capture2_pcm_copy finish3, copy size_2:%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x u4DataRemained:%x \r\n",
		     size_2, Vul_Block->u4DMAReadIdx, Vul_Block->u4WriteIdx,
		     Vul_Block->u4DataRemained);
	}
