...
static uint32_t GetFmUplinkSamplingRate(void)
{
	return kFmUplinkSamplingRateHz;
}

static uint32_t GetFmDownlinkSamplingRate(void)
{
...
static int mtkalsa_fmi2s2_constraints(struct snd_pcm_runtime *runtime)
{
	...
	if (err < 0) {
		pr_debug("mtkalsa_fmi2s2_constraints SNDRV_PCM_HW_PARAM_PERIODS err = %d", err);
		return err;
	}
...
{
	pr_debug("mtk_fmi2s2_open\n");

	struct snd_pcm_runtime *runtime = substream->runtime;
	int err = 0;
	int ret = 0;
	runtime->hw = mtk_fm_i2s2_hardware;
	memcpy((void *)(&(runtime->hw)), (void *)&mtk_fm_i2s2_hardware,
	       sizeof(struct snd_pcm_hardware));

	pr_debug("runtime->hw->rates= 0x%x mtk_fm_i2s2_hardware = = 0x%x\n ", runtime->hw.rates,
		 &mtk_fm_i2s2_hardware);
	...
	pr_debug("mtk_fmi2s2_open runtime rate = %d channels = %d substream->pcm->device = %d\n",
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		pr_debug("SNDRV_PCM_STREAM_PLAYBACK mtkalsa_fmi2s2_constraints\n");
	} else {
		return -1;
	}

	if (err < 0) {
		pr_debug("mtk_fm_i2s2_close\n");
		mtk_fm_i2s2_close(substream);
		return err;
	}
	pr_debug("mtk_fmi2s2_open return\n");
	return 0;
}
			...
			void __user *dst, snd_pcm_uframes_t count)
{
	...
	pr_debug("mtk_pcm_copy pos = %d count = %d\n ", pos, count);
	count = count << 2;
	return 0;
}
...
static int mtk_fmi2s2_prepare(struct snd_pcm_substream *substream)
{
	...
	pr_debug("mtk_fmi2s2_prepare rate = %d  channels = %d period_size = %d\n",
	...
	SetHwDigitalGainMode(Soc_Aud_Hw_Digital_Gain_HW_DIGITAL_GAIN2,
			     Soc_Aud_I2S_SAMPLERATE_I2S_44K, 0xC8);
...
static int mtk_fmi2s2_hwgain_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = Fm_Hw_gain;
	pr_debug("%s: hw_gain=%d\n", __func__, Fm_Hw_gain);
	return 0;
}

static int mtk_fmi2s2_hwgain_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static int mtk_fm_i2s2_probe(struct platform_device *pdev)
{
	...
	return snd_soc_register_platform(&pdev->dev, &mtk_soc_fmi2s2_platform);
}
...
static int mtk_fm_i2s2_platform_probe(struct snd_soc_platform *platform)
{
	pr_debug("mtk_fm_i2s2_platform_probe\n");
