...
*/
	...
	if (prMsduInfo->fgIsTXDTemplateValid && (prMsduInfo->ucControlFlag == 0) && prStaRec) {
		...
		kalMemCopy(prTxDesc, prTxDescTemplate, *pucTxDescLength);
...
*/
	...
	HAL_MAC_TX_DESC_SET_TX_BYTE_COUNT(prTxDesc,
					  *pucTxDescLength + NIC_TX_DESC_PADDING_LENGTH +
...
WLAN_STATUS nicTxCmd(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN UINT_8 ucTC)
{
	...
	if (prCmdInfo->eCmdType == COMMAND_TYPE_SECURITY_FRAME) {
		...
		if (prStaRec) {
			DBGLOG(TX, INFO,
			       ("TX SEC Frame: BSS[%u] STA_REC[%u] WLAN_IDX[%u] LEN[%u]\n",
				prCmdInfo->ucBssIndex, prStaRec->ucIndex, prStaRec->ucWlanIndex,
				ucTxDescAndPaddingLength + prCmdInfo->u2InfoBufLen));
		} else {
			DBGLOG(TX, INFO,
			       ("TX SEC Frame: BSS[%u] STA_REC[%u] WLAN_IDX[DEFAULT] LEN[%u]\n",
				prCmdInfo->ucBssIndex, STA_REC_INDEX_NOT_FOUND,
				ucTxDescAndPaddingLength + prCmdInfo->u2InfoBufLen));
		}
