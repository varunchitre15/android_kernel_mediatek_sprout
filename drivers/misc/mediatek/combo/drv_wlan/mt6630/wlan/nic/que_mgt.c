		...
		if (IS_BSS_ACTIVE(prBssInfo)) {
			...
			default:
				...
				do {
					...
					if (prStaRec->fgIsQoS) {
						if (prCurrentMsduInfo->ucUserPriority <
						    TX_DESC_TID_NUM) {
							eAci =
							    aucTid2ACI[prCurrentMsduInfo->
								       ucUserPriority];
							prTxQue =
							    &prStaRec->
							    arTxQueue[aucACI2TxQIdx[eAci]];
							ucTC =
							    arNetwork2TcResource[prCurrentMsduInfo->
										 ucBssIndex][eAci];
						} else {
			...
			if ((prCurrentMsduInfo->eSrc == TX_PACKET_FORWARDING)
			    && (prTxQue->u4NumElem > QM_FWD_PKT_QUE_THRESHOLD)) {
				DBGLOG(QM, TRACE,
				   ...
				   IN UINT_32 u4CurrentQuota, IN UINT_32 u4TotalQuota)
{
	...
	while (TRUE) {
		if (!(prStaRec->fgIsTxAllowed) || (ucPktCount == 0) || QUEUE_IS_EMPTY(prCurrQueue)) {
			break;
...
qmDequeueTxPacketsFromPerTypeQueues(IN P_ADAPTER_T prAdapter,
				    OUT P_QUE_T prQue, IN UINT_8 ucTC, IN UINT_32 u4CurrentQuota)
{
	P_BSS_INFO_T prBssInfo;
	P_QUE_T prCurrQueue;
	...
	DBGLOG(QM, LOUD,
	       ("Enter qmDequeueTxPacketsFromPerTypeQueues (TC = %d, quota = %u)\n", ucTC,
		u4CurrentQuota));
...
P_MSDU_INFO_T qmDequeueTxPackets(IN P_ADAPTER_T prAdapter, IN P_TX_TCQ_STATUS_T prTcqStatus)
{
	...
	qmDequeueTxPacketsFromPerTypeQueues(prAdapter,
					    &rReturnedQue,
					    TC5_INDEX, prTcqStatus->au2FreePageCount[TC5_INDEX]);
			...
			/* Check Class Error */
			{
				if (secCheckClassError
					...
					if (prReorderQueParm && prReorderQueParm->fgIsValid
						...
						if (!fgIsBMC
						    && nicRxIsDuplicateFrame(prCurrSwRfb) == TRUE) {
							DBGLOG(QM, TRACE,
							       ("Duplicated packet is detected\n"));
							prCurrSwRfb->eDst = RX_PKT_DESTINATION_NULL;
						}
						/* ToDo[6630]: defragmentation */
						if (prCurrSwRfb->fgFragFrame) {
							prCurrSwRfb =
							    incRxDefragMPDU(prAdapter, prCurrSwRfb,
									    prReturnedQue);
							if (prCurrSwRfb) {
								prRxStatus =
					...
					case VENDOR_OUI_SUBTYPE_WMM_INFO:
						{
							...
							prStaRec->fgIsUapsdSupported =
							    ...
							    IS_FEATURE_ENABLED(prAdapter->rWifiVar.
									       ucUapsd);
...
mqmGenerateWmmInfoIEByParam(BOOLEAN fgSupportUAPSD,
			    ...
			    UINT_8 ucBmpTriggerAC, UINT_8 ucUapsdSp, UINT_8 *pOutBuf)
{
	P_IE_WMM_INFO_T prIeWmmInfo;
	UINT_32 ucUapsd[] = {
...
mqmGenerateWmmInfoIEByStaRec(P_ADAPTER_T prAdapter,
			     P_BSS_INFO_T prBssInfo, STA_RECORD_T *prStaRec, UINT_8 *pOutBuf)
{
	P_IE_WMM_INFO_T prIeWmmInfo;
	UINT_32 ucUapsd[] = {
		WMM_QOS_INFO_BE_UAPSD,
		 ...
		 IN P_MSDU_INFO_T prMsduInfo, IN ENUM_FRAME_TYPE_IN_CMD_Q_T eFrameType)
{
	...
	if (!IS_BSS_ACTIVE(prBssInfo)) {
		...
		DBGLOG(QM, INFO,
		       ("Drop packets Action (Inactive BSS %u).\n", prBssInfo->ucBssIndex));
		TX_INC_CNT(&prAdapter->rTxCtrl, TX_INACTIVE_BSS_DROP);
