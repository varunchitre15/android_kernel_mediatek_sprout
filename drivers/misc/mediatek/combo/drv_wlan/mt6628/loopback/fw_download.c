...
int firmware_download(char *filename, MTK_WCN_HIF_SDIO_CLTCTX cltCtx) {
	...
	if(ValidHead == 1) {
		for(i = 0 ; i < prFwHead->u4NumOfEntries ; i++) {
			for(j = 0 ; j < prFwHead->arSection[i].u4Length ; j += CMD_PKT_SIZE_FOR_IMAGE) {
				...
				prInitHifTxHeader->u2TxByteCount = ALIGN_4(sizeof(INIT_HIF_TX_HEADER_T) + sizeof(INIT_CMD_DOWNLOAD_BUF) + translen);
				...
				prInitCmdDownloadBuf->u4Address = prFwHead->arSection[i].u4DestAddr + j;
				prInitCmdDownloadBuf->u4Length = translen;
				prInitCmdDownloadBuf->u4CRC32 = 
						wlanCRC32((uint8_t *)(data + prFwHead->arSection[i].u4Offset + j),
										translen);
		...
		// upload firmware 
		while(len > 0) {
			int translen = (len > CMD_PKT_SIZE_FOR_IMAGE) ? CMD_PKT_SIZE_FOR_IMAGE : len;
	...
	prInitHifTxHeader->u2TxByteCount = ALIGN_4(sizeof(INIT_HIF_TX_HEADER_T) + sizeof(INIT_CMD_WIFI_START));
	prInitHifTxHeader->ucEtherTypeOffset = 0;
	prInitHifTxHeader->ucCSflags = 0;
...
int firmware_power_off(MTK_WCN_HIF_SDIO_CLTCTX cltCtx)
{
    ...
    mtk_wcn_hif_sdio_write_buf(cltCtx, MCR_WTDR1, (PUINT32)(prWifiCmd), sizeof(WIFI_CMD_T) + sizeof(CMD_NIC_POWER_CTRL));
