			  ...
			  struct thermal_cooling_device *cool_dev)
{
	...
	if (cool_dev == p_linux_if->cl_dev) {
		...
		wmt_tm_dprint("[wmt_thz_unbind] unbind device fail..!\n");
		return -EINVAL;
	}
#endif
	...
	if (thermal_zone_unbind_cooling_device(thz_dev, table_val, cool_dev)) {
		...
		wmt_tm_print("[wmt_thz_unbind] unbinding OK\n");
	}
	...
	*pv = 0;
	if (pg_wmt_tm) {
		...
		*pv = temp * 1000;
		...
		wmt_tm_dprint("[wmt_thz_get_temp] temp = %d\n", temp);
...
{
	wmt_tm_dprint("[mtktspa_get_trip_type] %d\n", trip);
...
{
	wmt_tm_dprint("[mtktspa_get_trip_temp] %d\n", trip);
	*pv = g_trip_temp[trip];
	return 0;
}
...
static int wmt_send_signal(int level)
{
	...
	if (tm_input_pid == 0) {
		wmt_tm_dprint("[%s] pid is empty\n", __func__);
		ret = -1;
	}

	wmt_tm_print("[%s] pid is %d, %d, %d\n", __func__, tm_pid, tm_input_pid, thro);
...
static int wmt_judge_throttling(int index, int is_on, int interval)
{
	...
	wmt_tm_dprint("[%s]+ [0]=%d, [1]=%d || [%d] is %s\n", __func__, mail_box[0], mail_box[1],
	...
	if (mail_box[0] >= 0 && mail_box[1] >= 0) {
		...
		case WFD_STAT:
			if (throttling_pre_stat != WFD_STAT) {
				...
				wmt_tm_print("WFD is on, reset everything!");
			}
			break;
		...
		case HIGH_STAT:
			if (throttling_pre_stat < HIGH_STAT || throttling_pre_stat == WFD_STAT) {
				...
				if ((over_up_time * interval) >= up_duration) {
					...
					wmt_tm_print("HIGH-->HIGH:%lu <- (%d / %d) %lu",
		...
		case MID_STAT:
			if (throttling_pre_stat == LOW_STAT) {
				...
				wmt_tm_print("[%s] Go up!!\n", __func__);
			} else if (throttling_pre_stat == HIGH_STAT) {
				over_up_time = 0;
				throttling_pre_stat = MID_STAT;
				wmt_tm_print("[%s] Go down!!\n", __func__);
			} else {
				throttling_pre_stat = MID_STAT;
				wmt_tm_dprint("[%s] pre_stat=%d!!\n", __func__,
					      throttling_pre_stat);
			}
			break;
		...
		case LOW_STAT:
			if (throttling_pre_stat == WFD_STAT) {
				...
				wmt_tm_print("MID/HIGH-->LOW:%lu <- (%d / %d) %lu", thro_constraint,
				...
				if ((below_low_time * interval) >= low_duration) {
					if (low_rst_time >= low_rst_max && !is_reset) {
						wmt_tm_print("over rst time=%d", low_rst_time);
						...
						wmt_tm_print("LOW-->LOW:%lu <-(%d / %d) %lu",
						...
						wmt_tm_dprint("Have reset, no control!!");
					}
				}
			} else {
			...
			wmt_tm_dprint("case0 time=%d, rst=%d %d\n", below_low_time, low_rst_time,
				      is_reset);
			break;
		...
		wmt_tm_dprint("[%s] dont get all info!!\n", __func__);
	}
	return 0;
}
...
static ssize_t wmt_tm_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
	...
	if (sscanf
		...
		wmt_tm_print("[wmt_tm_write] g_trip_temp [0]=%d, [1]=%d, [2]=%d, [3]=%d, [4]=%d\n",
			     g_thermal_trip[0], g_thermal_trip[1], g_thermal_trip[2],
			     g_thermal_trip[3], g_thermal_trip[4]);

		wmt_tm_print("[wmt_tm_write] g_trip_temp [5]=%d, [6]=%d, [7]=%d, [8]=%d, [9]=%d\n",
			     g_thermal_trip[5], g_thermal_trip[6], g_thermal_trip[7],
			     g_thermal_trip[8], g_thermal_trip[9]);

		wmt_tm_print("[wmt_tm_write] cooldev [0]=%s, [1]=%s, [2]=%s, [3]=%s, [4]=%s,\n",
			     g_bind0, g_bind1, g_bind2, g_bind3, g_bind4);

		wmt_tm_print("[wmt_tm_write] cooldev [5]=%s, [6]=%s, [7]=%s, [8]=%s, [9]=%s,\n",
			     g_bind5, g_bind6, g_bind7, g_bind8, g_bind9);

		wmt_tm_print("[wmt_tm_write] trip_temp [0]=%d, [1]=%d, [2]=%d, [3]=%d, [4]=%d\n",
			     trip_temp[0], trip_temp[1], trip_temp[2], trip_temp[3], trip_temp[4]);

		wmt_tm_print("[wmt_tm_write] trip_temp [5]=%d, [6]=%d, [7]=%d, [8]=%d, [9]=%d\n",
			     trip_temp[5], trip_temp[6], trip_temp[7], trip_temp[8], trip_temp[9]);

		wmt_tm_print("[wmt_tm_write] polling time=%d\n", p_linux_if->interval);
		...
		wmt_tm_print("[wmt_tm_write] time_ms=%d\n", p_linux_if->interval);
...
static int wmt_tm_ops_register(struct wmt_thermal_ctrl_ops *ops)
{
	...
	if (pg_wmt_tm) {
		...
		p_des = &pg_wmt_tm->wmt_if.ops;
		if (ops != NULL) {
			wmt_tm_print("[wmt_tm_ops_register] reg start ...\n");
			p_des->query_temp = ops->query_temp;
			p_des->set_temp = ops->set_temp;
			wmt_tm_print("[wmt_tm_ops_register] reg end ...\n");
		} else {
			p_des->query_temp = 0;
			p_des->set_temp = 0;
		}
...
int wmt_tm_init(struct wmt_thermal_ctrl_ops *ops)
{
	...
	wmt_tm_print("[wmt_tm_init] start -->\n");
...
int wmt_tm_init_rt(void)
{
	...
	wmt_tm_print("[wmt_tm_init_rt] start -->\n");

	err = wmt_tm_thz_cl_register();
	if (err)
		return err;

	wmt_tm_print("[wmt_tm_init_rt] end <--\n");
...
int wmt_tm_deinit_rt(void)
{
	...
	wmt_tm_print("[wmt_tm_deinit_rt] start -->\n");

	err = wmt_tm_thz_cl_unregister();
	if (err)
		return err;

	wmt_tm_print("[wmt_tm_deinit_rt] end <--\n");
