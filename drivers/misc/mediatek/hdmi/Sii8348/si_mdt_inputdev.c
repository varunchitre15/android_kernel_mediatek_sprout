...
int init_mdt_touchscreen(struct mhl_dev_context *dev_context)
{
	...
	input_set_abs_params(dev_touchscreen, ABS_MT_POSITION_X, 0, dev_context->mdt_devs.x_max, 0,
			     0);
	input_set_abs_params(dev_touchscreen, ABS_MT_POSITION_Y, 0, dev_context->mdt_devs.y_max, 0,
			     0);
			     ...
			     struct mdt_packet *keyboard_packet)
{
	...
	for (i = 0; i < 3; i++)
		input_report_key(dev_keyboard,
				 usb_kbd_keycode[i + 224], (keycodes_new[0] >> i) & 1);
...
static void mdt_toggle_keycode(struct input_dev *hid_device, unsigned char keycode)
{
	if (NULL == hid_device)
		return;
...
void generate_event_mouse(struct mhl_dev_context *dev_context, struct mdt_packet *mousePacket)
{
	...
	input_report_key(dev_mouse, BTN_LEFT, mousePacket->header & MDT_HDR_MOUSE_BUTTON_1);
	...
	if (mousePacket->header & MDT_HDR_MOUSE_BUTTON_2) {
		...
		dev_context->mdt_devs.prior_right_button = 0;
#else
	input_report_key(dev_mouse, BTN_RIGHT, mousePacket->header & MDT_HDR_MOUSE_BUTTON_2);
#endif
	input_report_key(dev_mouse, BTN_MIDDLE, mousePacket->header & MDT_HDR_MOUSE_BUTTON_3);

	input_report_rel(dev_mouse, REL_X, mousePacket->event.mouse.x_displacement);

	input_report_rel(dev_mouse, REL_Y, mousePacket->event.mouse.y_displacement);

	input_report_rel(dev_mouse, REL_WHEEL, mousePacket->event.mouse.z_displacement);
				    ...
				    struct mdt_packet *touchPacket)
{
	...
	prior_event =
	    (struct mdt_touch_history_t *)&(dev_context->mdt_devs.prior_touch_events[contactID]);
...
bool si_mhl_tx_mdt_process_packet(struct mhl_dev_context *dev_context, void *packet)
{
	...
	if (MDT_HDR_IS_KEYBOARD & mdt_event_packet->header) {
		...
		MHL_TX_DBG_INFO(dev_context, "Unsupported gaming controller " "event received\n");
	} else {
