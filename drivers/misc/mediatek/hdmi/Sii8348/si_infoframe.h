...
typedef enum {
	cnGraphics = 0, cnPhoto = 1, cnCinema = 2, cnGame = 3
} BitsContent_e;

typedef enum {
	aqLimitedRange = 0, aqFullRange = 1, aqReserved0 = 2, aqReserved1 = 3
} AviQuantization_e;
...
typedef enum {
	hvfNoAdditionalHDMIVideoFormatPresent = 0, hvfExtendedResolutionFormatPresent =
	    1, hvf3DFormatIndicationPresent = 2
} HDMI_Video_Format_e;

typedef enum {
	tdsFramePacking = 0x00, tdsTopAndBottom = 0x06, tdsSideBySide = 0x08
} _3D_structure_e;

typedef enum {
	tdedHorizontalSubSampling = 0x0, tdedQuincunxOddLeftOddRight =
	    0x4, tdedQuincunxOddLeftEvenRight = 0x5, tdedQuincunxEvenLeftOddRight =
	    0x6, tdedQuincunxEvenLeftEvenRight = 0x7
} ThreeDExtData_e;
...
typedef enum {
	...
	InfoFrameType_AVI, InfoFrameType_VendorSpecific, InfoFrameType_Audio
} InfoFrameType_e;
