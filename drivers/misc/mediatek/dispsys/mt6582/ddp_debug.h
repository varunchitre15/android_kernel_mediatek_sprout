...
#define DISP_IRQ(string, args...) if(irq_log) pr_debug("[DDP]"string,##args)  // default off
#define DISP_DBG(string, args...) if(dbg_log) pr_debug("[DDP]"string,##args)  // default off, use "adb shell "echo dbg_log:1 > sys/kernel/debug/dispsys" to enable
#define DISP_MSG(string, args...) pr_debug("[DDP]"string,##args)  // default on, important msg, not err
#define DISP_ERR(string, args...) pr_err("[DDP]error:"string,##args)  //default on, err msg
