...
static struct chip_pmu chip_unknown = { CPU_UNKNOWN, NULL, NULL, "Unkown CPU", 0, 0, 0 };
...
static int mips_pmu_hw_get_event_desc(int idx, int event, char *event_desc)
{
	...
	if (NULL == event_desc) {
		printk("event_desc is NULL\n");
		return -1;
	}

	for (i = 0; i < chip->max_reg_count; i++) {
		if (chip->desc[idx][i].event == event) {
			/* printk("%s %d idx: %d event: %d name: %s\n", __FUNCTION__, __LINE__, idx, event, chip->desc[idx][i].name); */
			strcpy(event_desc, chip->desc[idx][i].name);
			break;
		}
	}
