...
#define LOCAL_LOG_LEN    128
	char tmplog[LOCAL_LOG_LEN];
	...
	for (i = 0; i < ARRAY_SIZE(md->ch_ports); i++) {
		...
		if (!list_empty(&md->ch_ports[i])) {
			snprintf(tmplog, LOCAL_LOG_LEN, "CH%d ports:", i);
			list_for_each_entry(port, &md->ch_ports[i], entry) {
				snprintf(tmplog, LOCAL_LOG_LEN, "%s%s(%d/%d),  ", tmplog,
					 port->name, port->rx_ch, port->tx_ch);
			}
			CCCI_INF_MSG(md->index, CORE, "%s:\n", tmplog);
		}
	}
}
