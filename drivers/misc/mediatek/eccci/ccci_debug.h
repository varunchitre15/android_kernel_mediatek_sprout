...
do { \
    if (ccci_debug_enable == 1) \
	pr_debug("["tag"] (%d)" fmt, (idx+1), ##args); \
    else if (ccci_debug_enable == 2) \
	pr_notice("["tag"] (%d)" fmt, (idx+1), ##args); \
} while (0)

#define CCCI_INF_MSG(idx, tag, fmt, args...) pr_notice("["tag"] (%d)" fmt, (idx+1), ##args)
#define CCCI_ERR_MSG(idx, tag, fmt, args...) pr_err("[err/" tag"] (%d)" fmt, (idx+1), ##args)
#endif
...
#define CCCI_DUMP_COM_MSG(fmt, args...)         CCCI_INF_MSG(0, "dump", fmt, ##args)
