...
void ccci_mem_dump(void *start_addr, int len)
{
	...
	if (NULL == curr_p) {
		CCCI_DUMP_COM_MSG("NULL point to dump!\n");
		return;
	}
	if (0 == len) {
		CCCI_DUMP_COM_MSG("Not need to dump\n");
		return;
	}

	CCCI_DUMP_COM_MSG("Base: %08x\n", (unsigned int)start_addr);
	/* Fix section */
	for (i = 0; i < _16_fix_num; i++) {
		CCCI_DUMP_COM_MSG("%03X: %08X %08X %08X %08X\n",
...
void ccci_dump_req(struct ccci_request *req)
{
	/*
	   ...
	   char tmplog[LOCAL_LEN] = {0};
	   if(req && req->skb) {
	   len = req->skb->len;
	   CCCI_DUMP_COM_MSG("[ccci dump(%d)]:\n", len);
	   for(i=0; i<len && len<16; i++) {
	   snprintf(tmplog, LOCAL_LEN, "%s %02X", tmplog, *(req->skb->data+i));
	   }
	   CCCI_DUMP_COM_MSG("%s\n", tmplog);

	   memset(tmplog, 0x0, sizeof(tmplog));
	   for(i=16; i<len && len<32; i++) {
	   snprintf(tmplog, LOCAL_LEN, "%s %02X", tmplog, *(req->skb->data+i));;
	   }
	   CCCI_DUMP_COM_MSG("%s\n", tmplog);
	   }
