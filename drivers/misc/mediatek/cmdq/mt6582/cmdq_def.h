...
#undef CMDQ_GPR_SUPPORT

#define CMDQ_DUMP_GIC (0)
#define CMDQ_PROFILE_MMP (0)

#define CMDQ_K2_PORTING (0) /* temp option for K2 porting(will remove when late EP stage) */
...
#define CMDQ_MAX_HIGH_PRIORITY_THREAD_COUNT (0)	/* Thread that are high-priority (display threads) */
...
#define CMDQ_ARG_A_SUBSYS_MASK          (0x00C00000) /*22-23 bit: subsys encoding */
...
typedef enum CMDQ_SCENARIO_ENUM {
	CMDQ_SCENARIO_USER_SPACE = 0,	/* client from user space, so the cmd buffer is in user space. */

	/* below scenario use for debug & testcase only*/
	CMDQ_SCENARIO_DEBUG = 1,
	CMDQ_SCENARIO_DEBUG_PREFETCH = 2,
	CMDQ_SCENARIO_PRIMARY_DISP = 3,
	CMDQ_SCENARIO_PRIMARY_MEMOUT = 4,
	CMDQ_SCENARIO_PRIMARY_ALL= 5 ,
	CMDQ_SCENARIO_SUB_DISP=6,
	CMDQ_SCENARIO_TRIGGER_LOOP = 7,

	CMDQ_SCENARIO_JPEG_DEC = 8,
	CMDQ_SCENARIO_DISP_ESD_CHECK = 9,
...
typedef enum CMDQ_ENG_ENUM {
	...
	CMDQ_ENG_MDP_LSCI,	/* 10 */
	CMDQ_ENG_CMDQ,	/*11*/

	/* below part for testcase only */
	CMDQ_ENG_FAKE_DISP_OVL0,
	CMDQ_ENG_FAKE_DISP_OVL1,
