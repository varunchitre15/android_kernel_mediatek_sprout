...
int aes_legacy_enc(uchar *input_buf, uint32 input_len, uchar *output_buf, uint32 output_len)
{
	...
	if (input_len != output_len) {
		pr_err("[%s] error, input len should be equal to output len\n", MOD);
		return -1;
	}

	if (0 != input_len % CIPHER_BLOCK_SIZE) {
		pr_err("[%s] error, input len should be mutiple of %d bytes\n", MOD,
		       CIPHER_BLOCK_SIZE);
		return -1;
	}
...
int aes_legacy_init_key(uchar *key_buf, uint32 key_len)
{
	...
	if (KEY_256 != key_len) {
		pr_err("[%s] key size error (%d) (should be %d bytes)\n", MOD, key_len, KEY_256);
		return -1;
	}
