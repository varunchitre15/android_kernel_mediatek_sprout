...
{
        printk("[ETM DEBUG] run_show show tracer.state 0x%x\n",tracer.state);
			...
			const char *buf, size_t n)
{
	...
	if (unlikely(sscanf(buf, "%u", &value) != 1))
		return -EINVAL;
        printk("[ETM DEBUG] run_show store value 0x%x\n",value);
        if(value == 1 ||value == 0)
        {
          printk ("[ETM DEBUG] Let AEE Skip run ETM/stop ETM\n");
          return n;
        }
	if (value == 2) {
		trace_start();
        printk("[ETM DEBUG] start() return\n");
	} else if(value == 3) {
		trace_stop();
        printk("[ETM DEBUG] stop() return\n");
        //data = vmalloc(8192);
        //if(data) {
...
{
	int v, etb_legnth;
    printk("[ETM DEBUG] etb_length_show\n");
...
static ssize_t etb_length_store(struct device *kobj, struct device_attribute *attr, 
			const char *buf, size_t n)
{
        printk("[ETM DEBUG] etb_length_store\n");
	/* do nothing */
	return n;
}
...
static inline ssize_t trace_data_show(struct device *kobj, 
			struct device_attribute *attr, char *buf)
{
         printk("[ETM DEBUG] trace_data_show\n");
...
{
	unsigned int value;
         printk("[ETM DEBUG] trace_data_store\n");
...
static ssize_t trace_range_show(struct device *kobj, 
			struct device_attribute *attr, char *buf)
{
        printk("[ETM DEBUG] trace_range_show\n");
...
static ssize_t trace_range_store(struct device *kobj, struct device_attribute *attr, 
			const char *buf, size_t n)
{
       printk("[ETM DEBUG] trace_range_store\n");  
...
{
	unsigned int i;
        printk("[ETM DEBUG] etm_online_show\n"); 
	for (i = 0; i < tracer.nr_etm_regs; i++ ) {
			...
			const char *buf, size_t n)
{
	...
	unsigned char str[10];
        printk("[ETM DEBUG] etm_online_store\n");
...
static ssize_t etm_tcr_store(struct device *kobj, struct device_attribute *attr, 
			const char *buf, size_t n)
{
        printk("[ETM DEBUG] etm_tcr_store\n");
	return n;
}
...
static ssize_t etm_idr0_show(struct device *kobj, 
			struct device_attribute *attr, char *buf)
{
        printk("[ETM DEBUG] etm_idr0_show\n");
...
static ssize_t etm_idr0_store(struct device *kobj, struct device_attribute *attr, 
			const char *buf, size_t n)
{
        printk("[ETM DEBUG] etm_idr0_store\n");
	return n;
}
...
static ssize_t etm_idr2_show(struct device *kobj, 
			struct device_attribute *attr, char *buf)
{
         printk("[ETM DEBUG] etm_idr2_show\n");
...
static ssize_t etm_idr2_store(struct device *kobj, struct device_attribute *attr, 
			const char *buf, size_t n)
{
         printk("[ETM DEBUG] etm_idr2_store\n");
	return n;
}
...
static ssize_t etm_lock_show(struct device *kobj, 
            struct device_attribute *attr, char *buf)
{
    printk("[ETM DEBUG] etm_lock_show\n");
    return 0;
}
...
{
	unsigned int value;
    printk("[ETM DEBUG] etm_lock_store\n");
    int i;
