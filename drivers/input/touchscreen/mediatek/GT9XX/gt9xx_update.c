...
static s8 gup_update_config(struct i2c_client *client)
{
	...
	if (IS_ERR_OR_NULL == update_msg.cfg_file) {
...
static u8 gup_check_update_file_fl(struct i2c_client *client, st_fw_head * fw_head, char *path)
{
	...
	for (i = FW_HEAD_LENGTH; i < update_msg.fw_total_len; i += 2) {
		fw_checksum +=
		    (gtp_hotknot_calibration_section0[i] << 8) +
		    gtp_hotknot_calibration_section0[i + 1];
	}
...
static u8 gup_download_fw_ss51(struct i2c_client *client, u8 dwn_mode)
{
	...
	ret = i2c_write_bytes(client, 0xC000, &gtp_hotknot_calibration_section0[FW_HEAD_LENGTH], FW_DOWNLOAD_LENGTH);	// write the first bank
...
static u8 gup_download_fw_dsp(struct i2c_client *client, u8 dwn_mode)
{
	...
	ret = i2c_write_bytes(client, 0xC000, &gtp_hotknot_calibration_section0[FW_HEAD_LENGTH + 2 * FW_DOWNLOAD_LENGTH], FW_DSP_LENGTH);	// write the second bank
	if (ret == -1) {
...
static void gup_bit_write(s32 addr, s32 bit, s32 val)
{
	...
	if (ret != 0) {
		GTP_ERROR("I2C read misctl__RG_DMY failed\n");
		return;
	}
...
u8 gup_clk_dac_setting(int dac)
{
	s8 buf1, buf2;
	int ret;
	ret = i2c_read_bytes(i2c_client_point, _wRW_MISCTL__RG_DMY, &buf1, 1);
	if (ret != 0) {
		GTP_ERROR("I2C read misctl__RG_DMY failed\n");
		return -1;
	}
	ret = i2c_read_bytes(i2c_client_point, _bRW_MISCTL__RG_OSC_CALIB, &buf2, 1);
	if (ret != 0) {
		GTP_ERROR("I2C read misctl__RG_OSC_CALIB failed\n");
		return -1;
	}
...
u8 *gup_load_calibration_from_file(char *filepath)
{
	...
	GTP_INFO("[load_calibration_file]Load from file success: %ld %s", len, filepath);
