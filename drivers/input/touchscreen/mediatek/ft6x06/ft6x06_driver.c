...
static void tinno_update_tp_button_dim(int panel_vendor)
{
    if ( FTS_CTP_VENDOR_NANBO == panel_vendor ){
        tpd_button_setting(TPD_KEY_COUNT, touch_cust_ssb_data.touch_ssb_data[2].tpd_key_local, touch_cust_ssb_data.touch_ssb_data[2].tpd_key_dim_local);
    }else{
        tpd_button_setting(TPD_KEY_COUNT, touch_cust_ssb_data.touch_ssb_data[2].tpd_key_local, touch_cust_ssb_data.touch_ssb_data[2].tpd_key_dim_local);
    }
}
...
static void fts_6x06_hw_init(void)
{
    ...
    hwPowerOn(touch_cust_ssb_data.touch_ssb_data[2].power_id, VOL_2800, "TP");
#else
    hwPowerOn(touch_cust_ssb_data.touch_ssb_data[2].power_id, VOL_2800, "TP");
#endif
...
//END<touch panel><date20131021><tp auto update>yinhuiyong
 ...
 static int __devinit tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
 {
    ...
    if(touch_cust_ssb_data.touch_ssb_data[2].use_tpd_buttom == 1)
